...
export default function Withdraw({ onOutput }) {
  const { accounts, withdraw } = useBank();
  ...
  function handleSubmit(e) {
    ...
    if (result.success) {
      onOutput?.('success', `Withdrew ₹${Number(amount).toFixed(2)} from ${accountNo}. New balance: ₹${result.newBalance.toFixed(2)}`);
      setAmount('');
    } else {
